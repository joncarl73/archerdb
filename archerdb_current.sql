...
CREATE TABLE IF NOT EXISTS `event_checkins` (
  ...
  `participant_name` varchar(160) DEFAULT NULL,
  ...
  `lane_slot` varchar(10) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `event_participants` (
  ...
  `uses_wheelchair` tinyint(1) NOT NULL DEFAULT 0,
  `line_time_id` bigint(20) unsigned DEFAULT NULL,
  `assigned_lane` int(10) unsigned DEFAULT NULL,
  `assigned_slot` varchar(8) DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  ...
  KEY `event_participants_event_id_email_index` (`event_id`,`email`),
  KEY `event_participants_event_id_division_name_bow_type_index` (`event_id`,`division_name`,`bow_type`),
...
-- Dumping structure for table archer_db.event_scores
CREATE TABLE IF NOT EXISTS `event_scores` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `event_id` bigint(20) unsigned NOT NULL,
  `event_line_time_id` bigint(20) unsigned NOT NULL,
  `event_participant_id` bigint(20) unsigned NOT NULL,
  `arrows_per_end` tinyint(3) unsigned NOT NULL DEFAULT 3,
  `ends_planned` tinyint(3) unsigned NOT NULL DEFAULT 10,
  `scoring_system` varchar(32) NOT NULL DEFAULT '10',
  ...
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_event_line_participant` (`event_line_time_id`,`event_participant_id`),
  KEY `event_scores_event_id_foreign` (`event_id`),
  KEY `event_scores_event_participant_id_foreign` (`event_participant_id`),
  CONSTRAINT `event_scores_event_id_foreign` FOREIGN KEY (`event_id`) REFERENCES `events` (`id`) ON DELETE CASCADE,
  CONSTRAINT `event_scores_event_line_time_id_foreign` FOREIGN KEY (`event_line_time_id`) REFERENCES `event_line_times` (`id`) ON DELETE CASCADE,
  CONSTRAINT `event_scores_event_participant_id_foreign` FOREIGN KEY (`event_participant_id`) REFERENCES `event_participants` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Data exporting was unselected.

-- Dumping structure for table archer_db.event_score_ends
CREATE TABLE IF NOT EXISTS `event_score_ends` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `event_score_id` bigint(20) unsigned NOT NULL,
  ...
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_event_score_end` (`event_score_id`,`end_number`),
  CONSTRAINT `event_score_ends_event_score_id_foreign` FOREIGN KEY (`event_score_id`) REFERENCES `event_scores` (`id`) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS `participant_imports` (
  ...
  CONSTRAINT `chk_import_owner` CHECK (`league_id` is not null and `event_id` is null or `league_id` is null and `event_id` is not null)
...
CREATE TABLE IF NOT EXISTS `rulesets` (
  ...
  `lane_count` int(10) unsigned NOT NULL DEFAULT 10,
  `scoring_values` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin DEFAULT NULL CHECK (json_valid(`scoring_values`)),
  `x_value` smallint(5) unsigned DEFAULT NULL,
