...
new class extends Component
{
    ...
    // pagination
    protected string $pageName = 'eventAccessPage';
    ...
    protected function refreshCompanyMembers(): void
    {
        ...
        $collaboratorIds = $this->event
            ->collaborators()
            ->pluck('users.id')
            ->all();

        $ownerId = optional($this->event->company)->owner_user_id;
    ...
    public function addCollaborator(): void
    {
        ...
        if ($user->company_id !== $this->event->company_id) {
        ...
        if ($this->event->collaborators()->where('users.id', $user->id)->exists()) {
            $this->dispatch('toast', type: 'info', message: 'That user is already a collaborator.');

            return;
        }

        $this->event->collaborators()->syncWithoutDetaching([$user->id => ['role' => $this->collabRole]]);
    ...
    public function changeRole(int $userId, string $role): void
    {
        ...
        if ($role !== 'owner') {
            $isOwner = $this->event->collaborators()->where('users.id', $userId)->wherePivot('role', 'owner')->exists();
            $ownerCount = $this->event->collaborators()->wherePivot('role', 'owner')->count();
        ...
        $this->event->collaborators()->updateExistingPivot($userId, ['role' => $role]);
    ...
    public function removeCollaborator(int $userId): void
    {
        ...
        $this->event->collaborators()->detach($userId);
...
<section class="w-full">
  <div class="mx-auto max-w-7xl">
    ...
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        ...
        <p class="mt-2 text-sm text-gray-700 dark:text-gray-300">
          Share this event with owners or managers. Company owners have full access automatically.
        </p>
      </div>
      <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
        <flux:button as="a" href="{{ route('corporate.events.index') }}" variant="ghost">
          ← Back to events
        </flux:button>
      </div>
    </div>
    ...
    {{-- Table (matches Leagues access page) --}}
