...
new class extends Component
{
    ...
    public function mount(Event $event, ParticipantImport $import): void
    {
        Gate::authorize('update', $event);

        // Must belong to this event (and not to a league)
        abort_if((int) $import->event_id !== (int) $event->id, 404);
        abort_if(! is_null($import->league_id), 404);
    }
};
?>

@php
    // Currency formatting (copying your league version)
...
<div class="space-y-6">
  {{-- Uniform title, matching league page --}}
  <h1 class="text-base font-semibold">Confirm Participant Import</h1>

  {{-- Details block: same gradient treatment as league --}}
  <div class="border border-gray-200 bg-gradient-to-br from-purple-50 via-indigo-50 to-blue-50 p-4 dark:from-purple-900/20 dark:via-indigo-900/20 dark:to-blue-900/20">
    <dl class="grid grid-cols-2 gap-4 text-base">
      <dt class="text-neutral-600 dark:text-neutral-300">Event</dt>
  ...
  {{-- CTA: identical button semantics, event route swap --}}
  <form method="POST" action="{{ route('corporate.events.participants.import.startCheckout', ['event'=>$event->id, 'import'=>$import->id]) }}">
    @csrf
    <flux:button
        type="submit"
        variant="primary"
  ...
  <p class="text-base text-neutral-600 dark:text-neutral-300">
    You’ll be redirected to Stripe Checkout. Once payment succeeds, we’ll automatically add these participants to your event.
